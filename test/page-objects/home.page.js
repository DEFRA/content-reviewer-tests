...
get errorHeader() {
  ...
  async waitForStatusCompleted(reviewName, timeoutMs = 600000) {
    ...
    while (Date.now() - start < timeoutMs) {
      ...
      const row =
        await this.getRowByReviewName(reviewName);
      ...
      const status =
        (await this.getStatusForRow(row).getText()).trim();
    ...
    const screenshot =
      `./test-results/status-timeout-${Date.now()}.png`;

    await browser.saveScreenshot(screenshot);

    throw new Error(
      `Status did not reach Completed for "${reviewName}"`
    );
  }
